...
/* users.dat format: username:password */
...
int Signup(const char *username, const char *password)
{
    ...
    {
        printf("[auth] Signup failed: invalid input\n");
        return 0;
    }

    if (username[0] == '\0' || password[0] == '\0')
    {
        printf("[auth] Signup failed: username/password cannot be empty\n");
        return 0;
    }
    ...
    while (fgets(line, sizeof(line), fp))
    {
                   ...
                   pw) == 2)
        {
                ...
                strcmp(pw, password) == 0)
            {
                ...
                strncpy(loggedInUser,
                        username,
                        sizeof(loggedInUser) - 1);
...
int ChangePassword(const char *username,
                   const char *old_pw,
                   const char *new_pw)
{
    if (username == NULL ||
           ...
           total < 500)
    {
        ...
        if (sscanf(line,
                   ...
                   existing,
                   pw) == 2 &&
            StrCaseCmp(existing, username) == 0 &&
            strcmp(pw, old_pw) == 0)
        {
            snprintf(lines[total],
                     MAX_LINE,
                     "%s:%s\n",
                     username,
                     new_pw);
